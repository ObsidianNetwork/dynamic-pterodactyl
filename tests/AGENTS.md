...
| Task | Start at | Notes |
|---|---|---|
| Harness boot / DB safety | `bootstrap.php`, `TestDatabaseGuard.php`, `../phpunit.xml` | `paymenter_test`, `:memory:`, or an atomically claimed new `dynamic-pterodactyl-test-*/database.sqlite` path is enforced before app boot |
...
- The bootstrap refuses unsafe databases and requires `APP_ENV=testing`. `DB_DATABASE` must be `paymenter_test`, `:memory:`, or `dynamic-pterodactyl-test-*/database.sqlite` under the system temp directory. For file-backed SQLite, the guard atomically creates a new private directory and database, holds the exclusive file handle, and rejects every pre-existing directory or database. `LaravelTestCase` migrates a newly claimed standalone SQLite database on its first application boot.
...
- Do not broaden the database guard beyond `paymenter_test`, `:memory:`, or an atomically claimed new system-temp SQLite directory. Never infer disposability from a pre-existing filename.
