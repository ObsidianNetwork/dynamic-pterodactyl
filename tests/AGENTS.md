...
| Task | Start at | Notes |
|---|---|---|
| Harness boot / DB safety | `bootstrap.php`, `TestDatabaseGuard.php`, `../phpunit.xml` | `paymenter_test`, `:memory:`, or a strictly named disposable SQLite file in the system temp directory is enforced before app boot |
...
- The bootstrap refuses unsafe databases and requires `APP_ENV=testing`. `DB_DATABASE` must be `paymenter_test`, `:memory:`, or a `dynamic-pterodactyl-test-*.sqlite` file directly inside the system temp directory before Laravel boots. An empty value is rejected, and existing SQLite files must resolve inside that directory with exactly one filesystem link.
...
- Do not broaden the database guard beyond `paymenter_test`, `:memory:`, or the validated disposable system-temp SQLite naming contract.
